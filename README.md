...
Only two rows of `ec_csv` are kept: the referenced per-region STENS score
row (row 11, `EC_REFERENCED_STENS_ROW` in `pipeline.py` — its label includes
the participant's name, so it's picked by row position rather than a label
